...
for k:=0:3 do for l:=0:3 do for m:=0:3 do for n:=0:3 do Rieuddd(k,l,m,n) := trigsimp( df(gaudd(k,l,n),x(m)) - df(gaudd(k,l,m),x(n))
		+ for j:=0:3 sum ( gaudd(k,j,m)*gaudd(j,l,n) - gaudd(k,j,n)*gaudd(j,l,m) ) );

for k:=0:3 do for l:=0:3 do for m:=0:3 do for n:=0:3 do Riedddd(k,l,m,n):= trigsimp( for j:=0:3 sum gdd(k,j)*Rieuddd(j,l,m,n) );
