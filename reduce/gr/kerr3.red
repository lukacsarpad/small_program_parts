...
array guu(3,3);
dgtp := trigsimp( gdd(0,0)*gdd(3,3)-gdd(0,3)*gdd(3,0) ); 
detg := trigsimp( dgtp*gdd(1,1)*gdd(2,2) );

guu(0,0) := trigsimp( gdd(3,3) / dgtp );
guu(0,3) := guu(3,0) := - trigsimp( gdd(0,3) / dgtp );
guu(3,3) := trigsimp( gdd(0,0) / dgtp );
guu(1,1) := 1/gdd(1,1);
guu(2,2) := 1/gdd(2,2);


rdetg:=sqrt(-detg);
rdetg:=sub(abs(sin(theta))=sin(theta),rdetg);
...
for k:=0:3 do for l:=0:3 do for m:=0:3 do gaddd(k,l,m) := trigsimp( 1/2 * ( df(gdd(k,l),x(m)) + df(gdd(k,m),x(l)) - df(gdd(l,m),x(k)) ) );

for k:=0:3 do for l:=0:3 do for m:=0:3 do gaudd(k,l,m) := trigsimp( for n:=0:3 sum guu(k,n)*gaddd(n,l,m) );

%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Riemann tensor %%%%%%%%%%%%%%%%%%%%%%%%%%%%
array Rieuddd(3,3,3,3), Riedddd(3,3,3,3);

% LL 92.1, faster according to MacCallum (no guu derivatives)
for j:=0:3 do for k:=0:3 do for l:=0:3 do for m:=0:3 do Riedddd(j,k,l,m) := trigsimp(
	1/2*( df(gdd(j,m),x(k),x(l)) +df(gdd(k,l),x(j),x(m)) -df(gdd(j,l),x(k),x(m)) -df(gdd(k,m),x(j),x(l)) )
	+ for n:=0:3 sum for s:=0:3 sum gdd(n,s)*( gaudd(n,k,l)*gaudd(s,j,m) - gaudd(n,k,m)*gaudd(s,j,l) )
    );

for i1:=0:3 do for i2:=0:3 do for i3:=0:3 do for i4:=0:3 do Rieuddd(i1,i2,i3,i4) := trigsimp(
	for j1:=0:3 sum guu(i1,j1)*Riedddd(j1,i2,i3,i4)
    );
...
for n:=0:3 do for j:=0:3 do for k:=0:3 do for l:=0:3 do for m:=0:3 do DRieudddd(n,j,k,l,m) := trigsimp(
...
for k:=0:3 do for l:=0:3 do Ricdd(k,l) := trigsimp( for m:=0:3 sum Rieuddd(m,k,m,l) );

% verify formula, 92.7
array Ricddp(3,3);

for j:=0:3 do for k:=0:3 do Ricddp(j,k) := trigsimp( for l:=0:3 sum ( df(gaudd(l,j,k),x(l)) - df(gaudd(l,j,l),x(k))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Ricci scalar %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Rs := trigsimp( for k:=0:3 sum for l:=0:3 sum guu(k,l)*Ricdd(l,k) );


%%%%%%%%%%%%%%%%%%%%%%%%%%%% Einstein tensor %%%%%%%%%%%%%%%%%%%%%%%%%%%%
array Eidd(3,3), Eiud(3,3);

for k:=0:3 do for l:=0:3 do Eidd(k,l) := trigsimp( Ricdd(k,l) - 1/2 * gdd(k,l)*Rs );

for j:=0:3 do for k:=0:3 do Eiud(j,k) := trigsimp( for l:=0:3 sum guu(j,l)*Eidd(l,k) );
...
for j:=0:3 do for k:=0:3 do for l:=0:3 do for m:=0:3 do Wedddd(j,k,l,m) := trigsimp( Riedddd(j,k,l,m)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Invariants %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
array Rieuudd(3,3,3,3), Rieuuuu(3,3,3,3), rsdddd(3,3,3,3), rsuuuu(3,3,3,3), rsdduu(3,3,3,3);

for i1:=0:3 do for i2:=0:3 do for i3:=0:3 do for i4:=0:3 do Rieuudd(i1,i2,i3,i4) :=
	trigsimp( for j2:=0:3 sum Rieuddd(i1,j2,i3,i4)*guu(i2,j2) );

for i1:=0:3 do for i2:=0:3 do for i3:=0:3 do for i4:=0:3 do Rieuuuu(i1,i2,i3,i4) :=
	trigsimp( for j3:=0:3 sum for j4:=0:3 sum Rieuudd(i1,i2,j3,j4)*guu(i3,j3)*guu(i4,j4) );

% dual of the Riemann tensor
for i1:=0:3 do for i2:=0:3 do for i3:=0:3 do for i4:=0:3 do rsdddd(i1,i2,i3,i4) :=
	trigsimp( for j1:=0:3 sum for j2:=0:3 sum 1/2*E4dddd(i1,i2,j1,j2)*Rieuudd(j1,j2,i3,i4) );

for i1:=0:3 do for i2:=0:3 do for i3:=0:3 do for i4:=0:3 do rsuuuu(i1,i2,i3,i4) := trigsimp(
	for j1:=0:3 sum for j2:=0:3 sum for j3:=0:3 sum for j4:=0:3 sum
	    rsdddd(j1,j2,j3,j4)*guu(i1,j1)*guu(i2,j2)*guu(i3,j3)*guu(i4,j4) );

for i1:=0:3 do for i2:=0:3 do for i3:=0:3 do for i4:=0:3 do rsdduu(i1,i2,i3,i4) := trigsimp(
	for j3:=0:3 sum for j4:=0:3 sum rsdddd(i1,i2,j3,j4)*guu(i3,j3)*guu(i4,j4) );

Invar1 := trigsimp( for j:=0:3 sum for k:=0:3 sum for l:=0:3 sum for m:=0:3 sum
    1/48*( Riedddd(j,k,l,m) * Rieuuuu(j,k,l,m) - i * Riedddd(j,k,l,m)*rsuuuu(j,k,l,m)) ) $ length(ws);
...
Invar2 := trigsimp( for j:=0:3 sum for k:=0:3 sum for l:=0:3 sum for m:=0:3 sum for p:=0:3 sum for s:=0:3 sum
